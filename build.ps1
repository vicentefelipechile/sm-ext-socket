# Build script for sm-ext-socket on Windows.
...
# The extension is built header-only against Boost.Asio (no compiled Boost
# libraries are needed), so --boost-path only needs to point at a Boost
# source/headers tree.
...
Write-Host '== sm-ext-socket Windows build ==' -ForegroundColor Cyan
...
# --- Resolve paths ---------------------------------------------------------
$smPath = Join-Path $ProjectRoot '..\sourcemod'
$boostRoot = Join-Path $ProjectRoot '..\boost-1.86.0'
if (-not (Test-Path $smPath)) {
    throw "SourceMod not found at $smPath"
}
if (-not (Test-Path (Join-Path $boostRoot 'boost\asio.hpp'))) {
    throw "Boost headers not found at $boostRoot"
}
